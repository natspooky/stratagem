...
import * as EC from './ENCORE_SEC_1.0.7v_branch.js';
import { STRATAGEM_LIST, STRATAGEM_KEYS } from './STRATAGEM_DATA.js';
...
function generateBuffer() {
	...
	let string =
		STRATAGEM_LIST[
			STRATAGEM_KEYS[
				Math.floor(Math.random() * (STRATAGEM_KEYS.length - 1))
			]
		].inputString;
...
function setup() {
	...
	json.forEach((strat) => {
		document.body.appendChild(EC.jsonElementify(strat));
	});
}
